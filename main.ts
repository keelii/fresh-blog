...
// }
//
// export const cfg = await setupConfig()
//
// if (cfg.isPrd()) {
//   await getCachedPosts(cfg.getEnv("POST_DIR"), true)
// }
//
// await start(manifest, { render });

import {setupConfig} from "./config/config.ts"
import {getCachedPosts, parseCachedYamlFile} from "./utils/post.ts";
...
if (cfg.isPrd()) {
  console.log(cfg.getEnv("POST_DIR"))
  await getCachedPosts(cfg.getEnv("POST_DIR"), true)
}
...
async function handler(_req: Request): Promise<Response> {
  ...
  try {
    if (url.pathname === "/favicon.ico") {
      const fd = await Deno.open(join(Deno.cwd(), "static/favicon.ico"))
      return Promise.resolve(new Response(fd.readable, {
